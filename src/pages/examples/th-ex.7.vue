...
export default {
  ...
  methods: {
    ...
    paintingRobot() {
      ...
      const body = this.paintEllipseBody(material1, 14, 4, 40);
      this.scene.add(body);

      // 手
      const leftHand = this.paintEllipse(material, 3, 1, 20);
      leftHand.position.x = -20;
      leftHand.position.y = 4;
      leftHand.rotateZ(-Math.PI * 0.08);
      this.scene.add(leftHand);
      this.rightHand = this.paintEllipse(material, 3, 1, 20);
      this.rightHand.position.x = 20;
      this.rightHand.position.y = 4;
      this.rightHand.rotateZ(Math.PI * 0.08);
      this.demo = new THREE.Group();
      this.object = new THREE.Object3D();
      this.object.add(this.rightHand);
      // this.object.position.y = -20;

      this.scene.add(this.object);
      this.demo.add(leftHand);
      const down = 10;
      const up = -20;
      leftHand.translateX(-up);
      this.demo.translateX(up);
      leftHand.translateY(-down);
      this.demo.translateY(down);
      this.scene.add(this.demo);
      // 头
      const head = new THREE.Group();
      const geometry3 = new THREE.SphereBufferGeometry(22, 32, 32);
      // geometry3.position.set(20, 20, 20);
    ...
    renderRun() { // 渲染过程
      ...
      // this.rightHand.rotateOnAxis(new THREE.Vector3(0, 0, 1).normalize(), 0.1);
      this.demo.rotation.z = Math.PI * this.index;
