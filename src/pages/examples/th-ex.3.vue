...
import { TrackballControls, OrbitControls } from '../../threeControl';
...
export default {
  ...
  methods: {
    ...
    lightInit() {
      const light = new THREE.PointLight('#fff', 1, 500);
    ...
    paintEllipseBody(material, header, food, height) {
      const groud = new THREE.Group();
      const geometry = new THREE.ConeBufferGeometry(header, 10, 32, 32, true);
      const sphere = new THREE.Mesh(geometry, material);
    ...
    paintingRobot() {
      // 身子
      const material1 = new THREE.MeshBasicMaterial({ color: '#fff', side: THREE.DoubleSide });
      const material = new THREE.MeshBasicMaterial({ color: '#2d8cf0', side: THREE.DoubleSide });
      ...
      rightHand.position.y = 10;
      rightHand.rotateZ(Math.PI * 0.04);
      this.scene.add(rightHand);
      // 头
      const head = new THREE.Group();
      const geometry3 = new THREE.SphereBufferGeometry(24, 32, 32);
      const sphere3 = new THREE.Mesh(geometry3, material);
      sphere3.position.y = 52;
      head.add(sphere3);

      // 眼睛
      const material2 = new THREE.MeshBasicMaterial({ color: '#fff', side: THREE.DoubleSide });
