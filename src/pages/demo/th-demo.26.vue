...
export default {
  ...
  methods: {
    ...
    geometryInit() {
      ...
      this.bulidGroup = new THREE.Group();
      this.bulidGroups = new THREE.Group();
      this.bulidGroup.add(mesh);
      meshed.position.y = 30;
      this.bulidGroup.add(meshed);
      this.bulidGroups.add(this.bulidGroup);
      this.scene.add(this.bulidGroups);
    ...
    raycastInit() {
      ...
      this.renderer.domElement.addEventListener('pointerdown', (event) => {
        const local_mouse = this.renderer.domElement.getBoundingClientRect();
        mouse.x = ((event.clientX - local_mouse.left) / this.width) * 2 - 1;
        mouse.y = -((event.clientY - local_mouse.top) / this.height) * 2 + 1;
        raycaster.setFromCamera(mouse, this.camera);
        const intersects = raycaster.intersectObject(this.bulidGroups, true);
        if (intersects.length > 0) {
          this.data_active = intersects;
        }
      });
      this.renderer.domElement.addEventListener('mousemove', (event) => {
        if (this.data_active) {
          const local_mouse = this.renderer.domElement.getBoundingClientRect();
          const resd = this.data_active.filter((res) => res && res.object)[0];
          console.log(resd);
          // this.data_active.forEach((item) => {
          //   item.position.x = ((event.clientX - local_mouse.left) / this.width) * 2 - 1;
          //   itemposition.z = -((event.clientY - local_mouse.top) / this.height) * 2 + 1;
          // });
        }
      });
    },
