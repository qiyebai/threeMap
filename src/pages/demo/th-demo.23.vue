  ...
  methods: {
    ...
    obstacle() {
      ...
      const local_texture = textureLoader.load('./images/building.jpg', (texture) => {
        ...
        this.renderObstacle(lineMaterial);
      });
      const triangleShape = new THREE.Shape();
      triangleShape.moveTo(8, 2);
      triangleShape.lineTo(4, 8);
      triangleShape.lineTo(12, 6);
      triangleShape.lineTo(12, 5);
      triangleShape.lineTo(15, 3);
      triangleShape.lineTo(13, 1);
      triangleShape.lineTo(8, 2); // close path
      const extrudeSettings = {
        depth: 16,
      };
      const geometry1 = new THREE.ExtrudeGeometry(triangleShape, extrudeSettings);
    ...
    renderObstacle(material) {
      ...
      const material2 = new THREE.MeshBasicMaterial({ color: 0x00ff00, shadowSide: THREE.DoubleSide });
      const geometry2 = new THREE.ExtrudeBufferGeometry(triangleShape, extrudeSettings);
    ...
    move(direction, value_object) {
      switch (direction) {
        case 'up': value_object.z += 1; this.control.target.z += 1; this.camera.position.z += 1; break;
        case 'down': value_object.z -= 1; this.control.target.z -= 1; this.camera.position.z -= 1; break;
        case 'left': value_object.x += 1; this.control.target.x += 1; this.camera.position.x += 1; break;
        case 'right': value_object.x -= 1; this.control.target.x -= 1; this.camera.position.x -= 1; break;
        default: break;
      }
    },
    planeInit() {
